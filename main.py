...
def index():
    ...
    <head>
        ...
        <style>
            :root{--primary:#00d4aa;--dark:#1a1a1a;--light:#f8f9fa;--text:#2c2c2c;--accent:#00a890;}
...
def sitemap():
    ...
    for p in posts:
        ...
        # Use only first 10 chars if it's a valid ISO date, else today's date
        date_str = p.pub_date[:10] if p.pub_date and len(p.pub_date) >= 10 and p.pub_date[4] == '-' and p.pub_date[7] == '-' else datetime.now().strftime("%Y-%m-%d")
