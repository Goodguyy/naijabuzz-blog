# main.py - NaijaBuzz FINAL 100% WORKING (2025) - ALL NEWS FIXED!
...
def index():
    ...
    if selected == 'all':
        # TRUE ALL NEWS — shows latest from EVERY category
        posts = Post.query.order_by(Post.pub_date.desc()).limit(90).all()
    else:
        # Case-insensitive match — fixes "All News" bug
...
def generate():
    ...
    for cat, url in FEEDS:
        try:
            f = feedparser.parse(url, request_headers={'User-Agent': 'NaijaBuzzBot/1.0'})
